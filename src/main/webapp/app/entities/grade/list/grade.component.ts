...
import { EntityArrayResponseType, GradeService } from '../service/grade.service';
...
export class GradeComponent implements OnInit {
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.grades = this.refineData(dataFromBody);
  }

  protected refineData(data: IGrade[]): IGrade[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IGrade[] | null): IGrade[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.gradeService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
